...
#define CS_MCP3208 2         // BCM_GPIO 2, 조도 센서
#define LEDBAR 26               // BCM_GPIO 26, led
#define SPI_CHANNEL 0         // SPI 통신 채널
#define SPI_SPEED 1000000 // 1Mhz, SPI 통신 속도
#define RGBLEDPOWER 24      // BCM_GPIO 24, RGB
#define RED 7                     // BCM_GPIO 7 - OUT
#define GREEN 8                  // BCM_GPIO 8 - OUT
#define BLUE 9                  // BCM_GPIO 9 - OUT
#define trigPin 1            // BCM_GPIO 1, 초음파 센서
#define echoPin 29            // BCM_GPIO 29, 초음파 센서
#define BUZCONTROL 28         // BCM_GPIO 28, Buzzer
#define FAN 22                     // BCM_GPIO 22, Fan
#define DHTPIN 11               // BCM_GPIO 11, 온습도 센서
...
void *threadlight(void *data)
{
   ...
   while (1)
   {
      ...
      if (ledvalue != 0)
      {
         sprintf(lightbuf, "curl '192.168.0.146:22 /DB/lightin.php?v=%u&l=%d'", adcValue_light, ledvalue);
         system(lightbuf);//192.168.0.146:22 
      }
...
void *threadrgb(void *data)
{
   ...
   while (1)
   {
      sprintf(buf, "curl -s '192.168.0.146:22 /DB/act_color.php'");
...
void *threadbuzzer(void *data)
{
   ...
   while (1)
   {
      ...
      if (distance < 10)
      {
         sprintf(disbuf, "curl '192.168.0.146:22 /DB/distancein.php?v=%u&b=on'", distance);
         system(disbuf);
         digitalWrite(BUZCONTROL, 1);
      }
      else
      {
         sprintf(disbuf, "curl '192.168.0.146:22 /DB/distancein.php?v=%u&b=off'", distance);
         system(disbuf);
...
void *threaddht22(void *data)
{
   ...
   while (1)
   {
      ...
      if ((j >= 40) && (dht22_dat[4] == ((dht22_dat[0] + dht22_dat[1] + dht22_dat[2] + dht22_dat[3]) & 0xFF)))
      {
         ...
         sprintf(dhtbuf, "curl '192.168.0.146:22 /DB/dhtin.php?h=%.2f&t=%.2f'", h, t);
         system(dhtbuf);
      }
      else
      {
